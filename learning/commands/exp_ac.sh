...
for seed in 10 11 12 13 14 15
    ...
    python run.py policy=gmmppo fixed_radius=true kl_radius=0.01 \
        dual_lr=0.001 task=AcrobotSwingup wandb_project=$wandb_project seed=$seed
...
for beta in -30 -20 -10 do
    ...
    python run.py policy=gmmppo fixed_radius=false \
        beta=$beta dual_lr=0.001 task=AcrobotSwingup wandb_project=$wandb_project seed=$seed
    done
done
